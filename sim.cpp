...
//	oss << p;
	...
	OptionalArgument<uint64> debugMemoryStart(&args, "debug_memory", "timestamp to start debugging output for the memories", numeric_limits<uint64>::max());
	OptionalArgument<uint64> debugCacheMemoryStart(&args, "debug_cache_memory", "timestamp to start debugging output for the cache memory", numeric_limits<uint64>::max());
	...
	OptionalArgument<uint64> debugHybridManagerStart(&args, "debug_hybrid_manager", "timestamp to start debugging output for the hybrid memory manager", numeric_limits<uint64>::max());
	OptionalArgument<uint64> debugHybridStart(&args, "debug_hybrid", "timestamp to start debugging output for the hybrid memory and the hybrid memory manager", numeric_limits<uint64>::max());
	...
	OptionalArgument<uint64> debugPolicyStart(&args, "debug_policy", "timestamp to start debugging output for the policy", numeric_limits<uint64>::max());

	OptionalArgument<uint64> progressPeriod(&args, "progress_period", "period in cycles used by the engine to print progress information (0 for no information)", 0);
	OptionalArgument<uint64> instrProgressPeriod(&args, "instr_progress_period", "period in number of instructions used by the engine to print progress information (0 for no information)", 10000000);
	...
	OptionalArgument<uint64> rollbackTimeout(&args, "rollback_timeout", "number of clock cycles after no rollback was started to try again", 10000);
	OptionalArgument<uint64> completeTimout(&args, "complete_timeout", "number of clock cycles after no complition was started to try again (0 means no completion)", 10000);
	OptionalArgument<uint64> updatePeriod(&args, "update_period", "monitor update period in number of cycles (0 means update on each demotion, rollback or completion)", 1000000);
	...
	OptionalArgument<string> partitionPeriodType(&args, "partition_period_type", "type of the partition recalculation period (cycles|instructions)", "cycles");
	...
	OptionalArgument<bool> rollback(&args, "rollback", "whether to enable rollback at the hybrid memory manager", false);
	...
	OptionalArgument<unsigned> completionThreshold(&args, "completion_threshold", "number of blocks left to migrate when the completion of the migration should be started", pageSize.getValue()/blockSize.getValue() - 1);
	...
	OptionalArgument<string> migrationPolicy(&args, "migration_policy", "migration policy (no_migration|multi_queue|new_policy|threshold|first_touch|double_clock|frequency|offline)", "multi_queue");
	...
	//Arguments for all migration policies
	...
	OptionalArgument<uint64> filterThreshold(&args, "filter_threshold", "filter threshold (currently not working)", 0); //0 means no filter
	...
	OptionalArgument<bool> rollbackAsDemotion(&args, "rollback_as_demotion", "whether to enable rollback as part of demotion in the Multi-Queue policy", false);
	...
	OptionalArgument<uint64> rollbackExpiration(&args, "rollback_expiration", "number of cycles since the start of migration that triggers its rollback (only when rollback not part of demotion)", 10000);


	//Arguments for the New Migration Policy
	OptionalArgument<uint64> addedBlocksThreshold(&args, "added_blocks_threshold", "extra number of blocks accessed added to the completion threshold", 0);
	OptionalArgument<uint64> rollbackThreshold(&args, "rollback_threshold", "number of cycles since the start of migration before a migration is considered for rollback", 1000);
	OptionalArgument<bool> rollbackLogicalTime(&args, "rollback_logical_time", "whether to use real time (clock cycles) or logical time (number of accesses) for rollback in the new migration policy", false);
	OptionalArgument<unsigned> numAgingPeriods(&args, "num_aging_periods", "number of aging periods for holding access counts", 10);
	OptionalArgument<uint64> agingPeriod(&args, "aging_period", "number of cycles or instructions of the period for aging access counts", 10000000);
	OptionalArgument<bool> agingLogicalTime(&args, "aging_logical_time", "whether to use real time (clock cycles) or logical time (number of accesses) for aging in the new migration policy", false);
	OptionalArgument<bool> npRollbackAsDemotion(&args, "np_rollback_as_demotion", "whether to enable rollback as part of demotion in the new policy", false);

	//Arguments for the Threshold Migration Policy
	OptionalArgument<uint64> numIntervals(&args, "num_intervals", "number of intervals to look into the past", 20);
	OptionalArgument<uint64> intervalLength(&args, "interval_length", "number of cycles each interval lasts", 100000);
...
//	OptionalArgument<uint64> migrationPeriod(&args, "migration_period", "migration period", 1);
	...
	//Arguments for old hybrid memory
...
//	OptionalArgument<uint64> pcmMigrationCost(&args, "pcm_migration_cost", "PCM migration cost", 1200);
	...
	OptionalArgument<MappingType> dramMappingType(&args, "dram_mapping_type", "DRAM mapping type (row_rank_bank_col_cha|row_col_rank_bank_cha|rank_bank_row_col_cha)", ROW_RANK_BANK_COL_CHA);
	...
	OptionalArgument<unsigned> dramChannels(&args, "dram_channels", "number of DRAM channels", 1);
	...
	OptionalArgument<MappingType> pcmMappingType(&args, "pcm_mapping_type", "PCM mapping type (row_rank_bank_col_cha|row_col_rank_bank_cha|rank_bank_row_col_cha)", ROW_COL_RANK_BANK_CHA);
	...
	OptionalArgument<unsigned> pcmChannels(&args, "pcm_channels", "number of PCM channels", 1);
	OptionalArgument<unsigned> pcmRanks(&args, "pcm_ranks", "number of PCM ranks per channel", 16); //4 DIMMs x 4 ranks per DIMM
...
//	return -1;
	...
	if (debugHybridStart.getValue() != numeric_limits<uint64>::max()){
		debugHybridMemoryStart.setValue(debugHybridStart.getValue());
		debugHybridManagerStart.setValue(debugHybridStart.getValue());
	}
	...
	if (debugCachesHybridStart.getValue() != numeric_limits<uint64>::max()){
		...
		debugHybridManagerStart.setValue(debugCachesHybridStart.getValue());
	}
	...
	if (debugStart.getValue() != numeric_limits<uint64>::max()){
		...
		debugMemoryStart.setValue(debugStart.getValue());
		debugCacheMemoryStart.setValue(debugStart.getValue());
		debugHybridMemoryStart.setValue(debugStart.getValue());
		debugHybridManagerStart.setValue(debugStart.getValue());
	}
	...
	IPartition *partition = 0;
	IMemoryManager *manager = 0;
	HybridMemoryManager *hmm = 0;
	OldHybridMemoryManager *ohmm = 0;

	if (memoryOrganization.getValue() == "dram"){
		dramMemory = new Memory("dram", "DRAM", &engine, &stats, debugMemoryStart.getValue(), DRAM_QUEUE, DRAM_OPEN, DRAM_ACCESS, DRAM_CLOSE, DRAM_BUS_QUEUE, DRAM_BUS, dramRowBufferPolicy.getValue(), DESTRUCTIVE_READS, dramMappingType.getValue(), dramGlobalQueue.getValue(), dramQueueSize.getValue(), dramChannels.getValue(), dramRanks.getValue(), dramBanksPerRank.getValue(), dramRowsPerBank.getValue(), dramBlocksPerRow.getValue(), blockSize.getValue(), dramOpenLatency.getValue(), dramCloseLatency.getValue(), dramAccessLatency.getValue(), false, dramBusLatency.getValue(), 0);
		manager = new SimpleMemoryManager(&stats, dramMemory, numProcesses, pageSize.getValue());
		memory = dramMemory;
	} else if (memoryOrganization.getValue() == "pcm"){
		pcmMemory = new Memory("pcm", "PCM", &engine, &stats, debugMemoryStart.getValue(), PCM_QUEUE, PCM_OPEN, PCM_ACCESS, PCM_CLOSE, PCM_BUS_QUEUE, PCM_BUS, pcmRowBufferPolicy.getValue(), NON_DESTRUCTIVE_READS, pcmMappingType.getValue(), pcmGlobalQueue.getValue(), pcmQueueSize.getValue(), pcmChannels.getValue(), pcmRanks.getValue(), pcmBanksPerRank.getValue(), pcmRowsPerBank.getValue(), pcmBlocksPerRow.getValue(), blockSize.getValue(), pcmOpenLatency.getValue(), pcmCloseLatency.getValue(), pcmAccessLatency.getValue(), pcmLongLatency.getValue(), pcmBusLatency.getValue(),0);
		manager = new SimpleMemoryManager(&stats, pcmMemory, numProcesses, pageSize.getValue());
		memory = pcmMemory;
	} else if (memoryOrganization.getValue() == "cache"){
		dramMemory = new Memory("dram", "DRAM", &engine, &stats, debugMemoryStart.getValue(), DRAM_QUEUE, DRAM_OPEN, DRAM_ACCESS, DRAM_CLOSE, DRAM_BUS_QUEUE, DRAM_BUS,  dramRowBufferPolicy.getValue(), DESTRUCTIVE_READS, dramMappingType.getValue(), dramGlobalQueue.getValue(), dramQueueSize.getValue(), dramChannels.getValue(), dramRanks.getValue(), dramBanksPerRank.getValue(), dramRowsPerBank.getValue(), dramBlocksPerRow.getValue(), blockSize.getValue(), dramOpenLatency.getValue(), dramCloseLatency.getValue(), dramAccessLatency.getValue(), false, dramBusLatency.getValue(),0);
		pcmMemory = new Memory("pcm", "PCM", &engine, &stats, debugMemoryStart.getValue(), PCM_QUEUE, PCM_OPEN, PCM_ACCESS, PCM_CLOSE, PCM_BUS_QUEUE, PCM_BUS, pcmRowBufferPolicy.getValue(), NON_DESTRUCTIVE_READS, pcmMappingType.getValue(), pcmGlobalQueue.getValue(), pcmQueueSize.getValue(), pcmChannels.getValue(), pcmRanks.getValue(), pcmBanksPerRank.getValue(), pcmRowsPerBank.getValue(), pcmBlocksPerRow.getValue(), blockSize.getValue(), pcmOpenLatency.getValue(), pcmCloseLatency.getValue(), pcmAccessLatency.getValue(), pcmLongLatency.getValue(), pcmBusLatency.getValue(),0);
		cacheMemory = new CacheMemory("cache_memory", "Cache Memory", &engine, &stats, debugCacheMemoryStart.getValue(), dramMemory, pcmMemory, dramCacheblockSize.getValue(), dramCacheAssoc.getValue(), CACHE_LRU, pageSize.getValue(), dramCacheTagPenalty.getValue(), dramCacheQueueSize.getValue());
		manager = new SimpleMemoryManager(&stats, pcmMemory, numProcesses, pageSize.getValue());
		memory = cacheMemory;
	...
	if (memoryOrganization.getValue() == "hybrid"){
		...
		if (partitionPolicy.getValue() == "none"){
			...
			pidsPerPolicy = 1;
			OfflinePartition* offline = new OfflinePartition(numProcesses, pageSize.getValue(), dramMemory->getSize(), counterTracePrefix.getValue(), counterTraceInfix.getValue(), "_0.trace", partitionPeriodType.getValue());
		...
		for (unsigned i = 0; i < partition->getNumPolicies(); i++){
			...
			if (migrationPolicy.getValue() == "no_migration"){
				policies.emplace_back(new NoMigrationPolicy(ossName.str(), &engine, debugPolicyStart.getValue(), partition->getDramPages(i), allocationPolicy.getValue(), pidsPerPolicy));
			} else if (migrationPolicy.getValue() == "multi_queue"){
				policies.emplace_back(new MultiQueueMigrationPolicy(ossName.str(), &engine, debugPolicyStart.getValue(), partition->getDramPages(i), allocationPolicy.getValue(), pidsPerPolicy, maxFreeDram.getValue(), numQueues.getValue(), thresholdQueue.getValue(), lifetime.getValue(), logicalTime.getValue(), filterThreshold.getValue(), secondDemotionEviction.getValue(), aging.getValue(), history.getValue(), pendingList.getValue(), rollbackAsDemotion.getValue(), promotionFilter.getValue(), demotionAttempts.getValue(), rollbackExpiration.getValue()));
			} else if (migrationPolicy.getValue() == "new_policy"){
				policies.emplace_back(new NewPolicy(ossName.str(), &engine, debugPolicyStart.getValue(), partition->getDramPages(i), allocationPolicy.getValue(), pidsPerPolicy, maxFreeDram.getValue(), addedBlocksThreshold.getValue(), rollbackThreshold.getValue(), rollbackLogicalTime.getValue(), numAgingPeriods.getValue(), agingPeriod.getValue(), agingLogicalTime.getValue(), npRollbackAsDemotion.getValue()));
			} else if (migrationPolicy.getValue() == "threshold"){
				policies.emplace_back(new ThresholdMigrationPolicy(ossName.str(), &engine, debugPolicyStart.getValue(), partition->getDramPages(i), allocationPolicy.getValue(), pidsPerPolicy, maxFreeDram.getValue(), numIntervals.getValue(), intervalLength.getValue()));
			} else  if (migrationPolicy.getValue() == "first_touch"){
//				policies.emplace_back(new FirstTouchMigrationPolicy(ossName.str(), &engine, debugPolicyStart.getValue(), partition->getDramPages(i), allocationPolicy.getValue(), allocator, pidsPerPolicy));
			} else  if (migrationPolicy.getValue() == "double_clock"){
//				policies.emplace_back(new DoubleClockMigrationPolicy(ossName.str(), &engine, debugPolicyStart.getValue(), partition->getDramPages(i), allocationPolicy.getValue(), allocator, pidsPerPolicy));
...
//				policies.emplace_back(new OfflineMigrationPolicy(ossName.str(), &engine, debugPolicyStart.getValue(), partition->getDramPages(i), allocationPolicy.getValue(), allocator, pidsPerPolicy, i, filename, metricType.getValue(), accessType.getValue(), weightType.getValue(), intervalCount.getValue(), metricThreshold.getValue()));
			} else {
				args.usage(cerr);
				return -1;
			}
		}
		hmm = new HybridMemoryManager(&engine, &stats, debugHybridManagerStart.getValue(), numCores, numProcesses, sharedL2, hybridMemory, policies, partition, blockSize.getValue(), pageSize.getValue(), flushPolicy.getValue(), flushQueueSize.getValue(), supressFlushWritebacks.getValue(), demoteTimout.getValue(), rollbackTimeout.getValue(), completeTimout.getValue(), updatePeriod.getValue(), partitionPeriod.getValue(), partitionPeriodType.getValue(), migrationTableSize.getValue(), rollback.getValue(), perPageStats.getValue(), perPageStatsFilename.getValue());
		manager = hmm;
	}
	...
	if (memoryOrganization.getValue() == "old_hybrid"){
		...
		for (unsigned i = 0; i < partition->getNumPolicies(); i++){
			...
			ossName << migrationPolicy.getValue() << "_policy_" << i;
			if (migrationPolicy.getValue() == "no_migration"){
				oldPolicies.emplace_back(new OldNoMigrationPolicy(ossName.str(), &engine, debugPolicyStart.getValue(), partition->getDramPages(i), allocationPolicy.getValue(), allocator, pidsPerPolicy));
			} else if (migrationPolicy.getValue() == "multi_queue"){
				oldPolicies.emplace_back(new OldMultiQueueMigrationPolicy(ossName.str(), &engine, debugPolicyStart.getValue(), partition->getDramPages(i), allocationPolicy.getValue(), allocator, pidsPerPolicy, numQueues.getValue(), thresholdQueue.getValue(), lifetime.getValue(), logicalTime.getValue(), filterThreshold.getValue(), secondDemotionEviction.getValue(), aging.getValue(), history.getValue(), pendingList.getValue()));
			} else  if (migrationPolicy.getValue() == "first_touch"){
				oldPolicies.emplace_back(new OldFirstTouchMigrationPolicy(ossName.str(), &engine, debugPolicyStart.getValue(), partition->getDramPages(i), allocationPolicy.getValue(), allocator, pidsPerPolicy));
			} else  if (migrationPolicy.getValue() == "double_clock"){
				oldPolicies.emplace_back(new OldDoubleClockMigrationPolicy(ossName.str(), &engine, debugPolicyStart.getValue(), partition->getDramPages(i), allocationPolicy.getValue(), allocator, pidsPerPolicy));
			} else if (migrationPolicy.getValue() == "frequency"){
				//monitoringStrategy.getValue(), monitorApp.getValue(), promotionPolicy.getValue(), demotionPolicy.getValue(), candidateListEvictionPolicy.getValue(), flushPolicy.getValue(), candidateListSize.getValue(), migrationQueueSize.getValue(), agingPeriod.getValue(), counterReadPeriod.getValue(), accessBitPeriod.getValue(),
			} else if (migrationPolicy.getValue() == "offline"){
				string filename = counterTracePrefix.getValue() + traceNames[i] + ".gz";
				oldPolicies.emplace_back(new OldOfflineMigrationPolicy(ossName.str(), &engine, debugPolicyStart.getValue(), partition->getDramPages(i), allocationPolicy.getValue(), allocator, pidsPerPolicy, i, filename, metricType.getValue(), accessType.getValue(), weightType.getValue(), intervalCount.getValue(), metricThreshold.getValue()));
			} else {
				args.usage(cerr);
				return -1;
			}
		}
		ohmm = new OldHybridMemoryManager(&engine, &stats, debugHybridManagerStart.getValue(), numCores, numProcesses, sharedL2, oldHybridMemory, oldPolicies, partition, blockSize.getValue(), pageSize.getValue(), migrationMechanism.getValue(), monitoringType.getValue(), monitoringLocation.getValue(), flushPolicy.getValue(), flushQueueSize.getValue(), supressFlushWritebacks.getValue(), partitionPeriod.getValue(), partitionPeriodType.getValue(), baseMigrationRate.getValue(), perPageStats.getValue(), perPageStatsFilename.getValue(), trace.getValue(), countersPrefix.getValue(), tracePeriod.getValue());
		manager = ohmm;
	}
	...
	for (unsigned i = 0; i < numCores; i++){
		...
		if (useCaches.getValue()){
			cpus[i] = new OOOCPU(&engine, ossName3.str(), ossDesc3.str(), debugCpuStart.getValue(), &stats, i, pid, manager, instrL1s[i], dataL1s[i], readers[i], blockSize.getValue(), instrLimit.getValue(), instrProgressPeriod.getValue(), robSize.getValue(), issueWidth.getValue());
		} else {
			cpus[i] = new OOOCPU(&engine, ossName3.str(), ossDesc3.str(), debugCpuStart.getValue(), &stats, i, pid, manager, memory, memory, readers[i], blockSize.getValue(), instrLimit.getValue(), instrProgressPeriod.getValue(), robSize.getValue(), issueWidth.getValue());
		}
		cpus[i]->start();
	}

	//Create aggregate CPU statistics
	AggregateStat<uint64, Sum<uint64>> sumCpuTotalTime(&stats, "sum_cpu_total_time", "Sum of total number of cycles across all CPUs");
	AggregateStat<uint64, Max<uint64>> maxCpuTotalTime(&stats, "max_cpu_total_time", "Maximum number of cycles across all CPUs");
	AggregateStat<uint64, Min<uint64>> minCpuTotalTime(&stats, "min_cpu_total_time", "Minumum number of cycles across all CPUs");
	AggregateStat<double, Avg<double, uint64>, uint64> avgCpuTotalTime(&stats, "avg_cpu_total_time", "Avegare number of cycles across all CPUs");
	AggregateStat<double, Stddev<double, uint64>, uint64> stddevCpuTotalTime(&stats, "stddev_cpu_total_time", "Standard deviatio of the number of cycles across all CPUs");
	for (unsigned i = 0; i < numCores; i++){
		auto stat = cpus[i]->getStatTotalTime();
		sumCpuTotalTime.addStat(stat);
		maxCpuTotalTime.addStat(stat);
		minCpuTotalTime.addStat(stat);
		avgCpuTotalTime.addStat(stat);
		stddevCpuTotalTime.addStat(stat);
	}
	...
	class Exit : public IEventHandler{
		void process(const Event * event) {
			//cout << engine.getTimestamp() << ": exiting due to stop event" << endl;
			exit(0);
		}
	};
	...
	if (memoryOrganization.getValue() == "dram"){
		...
		delete hmm;
		for (auto pit = policies.begin(); pit != policies.end(); ++pit){
			delete *pit;
		}
		delete partition;
		if (useCaches.getValue()){
		...
		delete ohmm;
		for (auto pit = oldPolicies.begin(); pit != oldPolicies.end(); ++pit){
			delete *pit;
		}
		delete partition;
		if (useCaches.getValue()){
