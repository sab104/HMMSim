...
HybridMemoryManager::HybridMemoryManager(
	...
	string perPageStatsFilenameArg
		...
		completeTimeout(completeTimeoutArg),
		updatePeriod(updatePeriodArg),
		...
		dramPartialMigrations(statCont, "manager_dram_partial_migrations", "Number of partial DRAM migrations (rollingBack)", 0),
		...
		onDemandDRAMFull(statCont, "manager_ondemand_dram_full", "Number of on demand migration attempts that failed due to the DRAM being full", 0),
		onDemandTableFull(statCont, "manager_ondemand_table_full", "Number of on demand migration attempts that failed due to the migration table being full", 0),
		onDemandAll(statCont, "manager_ondemand_all", "Number of on demand migration attempts", 0),
		...
		totalMigrationsPerPid(statCont, "manager_total_migrations_per_pid", "Number of total migrations by process", &dramMigrationsPerPid, &pcmMigrationsPerPid)
{
	...
	demoting = false;
	currentDemotePolicy = 0;

	rollingBack = false;
	currentRollbackPolicy = 0;

	completing = false;
	currentCompletePolicy = 0;
...
HybridMemoryManager::~HybridMemoryManager(){
	myassert(!demoting);
	myassert(!rollingBack);
	myassert(!completing);
	myassert(flushQueue.empty());
	myassert(stalledRequests.empty());
	myassert(tagChangeQueue.empty());
	for (unsigned i = 0; i < numProcesses; i++){
		myassert(stalledCpus[i].empty());
	}
	delete [] pages;
	delete [] stalledCpus;
}

bool HybridMemoryManager::access(unsigned pid, addrint virtualAddr, bool read, bool instr, addrint *physicalAddr, CPU *cpu){
	uint64 timestamp = engine->getTimestamp();
	//debug("(%u, %lu, %s, %s)", pid, virtualAddr, read ? "read" : "write", instr ? "instr" : "data");
	...
	if (it == pages[pid].end()){
		...
		debug("(%u, %lu, %s, %s)", pid, virtualAddr, read ? "read" : "write", instr ? "instr" : "data");
		debug("\tinserting into physical pages: (%lu; %u, %lu)", it->second.page, pid, virtualPage);
	...
	if(it->second.stallOnAccess){
		stalledCpus[pid][virtualPage].emplace(cpu);
...
bool HybridMemoryManager::migrateOnDemand(addrint physicalPage, addrint *destPhysicalPage){
	...
	if(migrationTableSize < maxMigrationTableSize){
		onDemandAll++;
		if (policies[pit->second.pid]->migrate(pit->second.pid, pit->second.virtualPage)){
			...
			*destPhysicalPage = dramFreePageList.front();
			...
			debug(": pid: %u, virtualPage: %lu, srcPhysPage: %lu, destPhysPage: %lu, dest: DRAM, state: COPY", pit->second.pid, pit->second.virtualPage, it->second.page, *destPhysicalPage);
			...
			//updateMonitor(pit->second.pid, pit->second.virtualPage, physicalPage);
...
void HybridMemoryManager::finish(unsigned core){
...
void HybridMemoryManager::allocate(const vector<string>& filenames){
	...
	vector<unsigned> toPolicy;
	vector<unsigned> toPid;
	for (unsigned pid = 0; pid < filenames.size(); pid++){
		ifs.emplace_back(new ifstream(filenames[pid]));
		if (partition->getNumPolicies() == 1){
			toPolicy.emplace_back(0);
		} else {
			toPolicy.emplace_back(pid);
		}
		toPid.emplace_back(pid);
	}
	addrint virtualPage;
	uint64 count = 0;
	unsigned index = 0;
	while (count < dramPages && !toPid.empty()){
		if (*ifs[index] >> virtualPage){
			unsigned policy = toPolicy[index];
			unsigned pid = toPid[index];
			PageType type = policies[policy]->allocate(pid, virtualPage, false, false);
			myassert(type == DRAM);
			myassert(!dramFreePageList.empty());
			...
			myassert(ins);
			count++;
			index = (index + 1) % toPid.size();
			debug(": allocated in DRAM for pid %u", pid);
		} else {
			ifs.erase(ifs.begin()+index);
			toPid.erase(toPid.begin()+index);
		}
	}
	while(!toPid.empty()){
		if (*ifs[index] >> virtualPage){
			...
			addrint freePage = pcmFreePageList.front();
			pcmFreePageList.pop_front();
			...
			myassert(ins);
			index = (index + 1) % toPid.size();
			debug(": allocated in PCM for pid %u", pid);
		} else {
			toPid.erase(toPid.begin()+index);
		}
	}
...
void HybridMemoryManager::process(const Event * event){
	...
	if (type == DEMOTE){
		...
		selectPolicyAndComplete();
	} else if (type == ROLLBACK){
		selectPolicyAndRollback();
	...
	} else if (type == UPDATE_MONITORS){
		if (coresFinished.size() != numCores){
			updateMonitors();
			addEvent(updatePeriod, UPDATE_MONITORS);
		}
...
void HybridMemoryManager::selectPolicyAndDemote(){
	...
	unsigned previousPolicy = currentDemotePolicy;
	do {
		currentDemotePolicy = (currentDemotePolicy + 1) % partition->getNumPolicies();
		demoting = startDemotion(currentDemotePolicy);
	} while(!demoting && previousPolicy != currentDemotePolicy);
	if (!demoting && coresFinished.size() != numCores) {
		//info("(): adding DEMOTE event for %lu", timestamp+demoteTimeout);
		addEvent(demoteTimeout, DEMOTE);
	}
}

bool HybridMemoryManager::startDemotion(unsigned policy){
	uint64 timestamp = engine->getTimestamp();
	debug("(%u)", policy);
	unsigned pid;
	addrint virtualPage;
	debug(": migrationTableSize: %u", migrationTableSize);
	if (migrationTableSize < maxMigrationTableSize && policies[policy]->demote(&pid, &virtualPage)){
		...
		if (it->second.isMigrating){
			...
			myassert(!mit->second.completing);
			mit->second.rolledBack = true;
			mit->second.originalDemoted = true;
			if (mit->second.state == COPY) {
				memory->rollback(mit->first);
			}

			debug(": rollback: pid: %d, virtualPage: %lu, srcPhysPage: %lu, destPhysPage: %lu, dest: PCM, state: %d, startMigrationTime: %lu", pid, virtualPage, it->second.page, mit->second.destPhysicalPage, mit->second.state, mit->second.startMigrationTime);
			...
			debug(": demotion: pid: %u, virtualPage: %lu, srcPhysPage: %lu, destPhysPage: %lu, dest: PCM, state: %d", pid, virtualPage, it->second.page, destPhysPage, state);
...
void HybridMemoryManager::selectPolicyAndComplete(){
	myassert(!completing);
	debug("(): migrationTableSize: %u, migrations.size(): %lu", migrationTableSize, migrations.size());
	if (updatePeriod == 0){
		updateMonitors();
	}
	unsigned previousPolicy = currentCompletePolicy;
	do {
		currentCompletePolicy = (currentCompletePolicy + 1) % partition->getNumPolicies();
		completing = startCompletion(currentCompletePolicy);
	} while(!completing && previousPolicy != currentCompletePolicy);
	if (!completing && coresFinished.size() != numCores){
		//uint64 timestamp = engine->getTimestamp();
		//info("added COMPLETE event");
		addEvent(completeTimeout, COMPLETE);
	}
}

bool HybridMemoryManager::startCompletion(unsigned policy){
	debug("(%u)", policy);
	unsigned pid;
	addrint virtualPage;
	if (policies[policy]->complete(&pid, &virtualPage)){
		PageMap::iterator it = pages[pid].find(virtualPage);
		myassert(it != pages[pid].end());
		myassert(isPcmPage(it->second.page));
		myassert(it->second.type == PCM);
		myassert(it->second.isMigrating);
		auto mit = migrations.find(it->second.page);
		myassert(mit != migrations.end());
		myassert(!mit->second.rolledBack);

		mit->second.completing = true;
		if (mit->second.state == COPY) {
			memory->complete(mit->first);
		}

		debug(": completion: pid: %d, virtualPage: %lu, srcPhysPage: %lu, destPhysPage: %lu, state: %d, startMigrationTime: %lu", pid, virtualPage, it->second.page, mit->second.destPhysicalPage, mit->second.state, mit->second.startMigrationTime);
...
void HybridMemoryManager::selectPolicyAndRollback(){
	myassert(!rollingBack);
	if (updatePeriod == 0){
		updateMonitors();
	}
	unsigned previousPolicy = currentRollbackPolicy;
	do {
		currentRollbackPolicy = (currentRollbackPolicy + 1) % partition->getNumPolicies();
		rollingBack = startRollback(currentRollbackPolicy);
	} while(!rollingBack && previousPolicy != currentRollbackPolicy);
	if (!rollingBack && coresFinished.size() != numCores) {
		addEvent(rollbackTimeout, ROLLBACK);
	}
}

bool HybridMemoryManager::startRollback(unsigned policy){
	uint64 timestamp = engine->getTimestamp();
	debug("(%u)", policy);
	unsigned pid;
	addrint virtualPage;
	if (policies[policy]->rollback(&pid, &virtualPage)){
		PageMap::iterator it = pages[pid].find(virtualPage);
		myassert(it != pages[pid].end());
		myassert(isPcmPage(it->second.page));
		myassert(it->second.type == PCM);
...
void HybridMemoryManager::updateMonitor(unsigned pid, addrint virtualPage, addrint physicalPage){
	monitors.clear();
	progress.clear();
	memory->readCountsAndProgress(physicalPage, &monitors, &progress);
	myassert(monitors.size() <= 1);
	if (monitors.size() == 1){
		monitors[0].pid = pid;
		monitors[0].page = virtualPage;
	}
	myassert(progress.size() <= 1);
	if (progress.size() == 1){
		progress[0].pid = pid;
		progress[0].page = virtualPage;
	}
	if (partition->getNumPolicies() == 1){
		policies[0]->monitor(monitors, progress);
	} else {
		policies[pid]->monitor(monitors, progress);
	}
}
...
void HybridMemoryManager::updateMonitors(){
	...
	//uint64 timestamp = engine->getTimestamp();
	//info(": progress.size(): %lu", progress.size());
	if (partition->getNumPolicies() == 1){
...
void HybridMemoryManager::flushCompleted(addrint addr, bool dirty, IMemory *caller){
	...
	auto it = flushQueue.begin();
	while (!flushScheduled && it != flushQueue.end() && flushQueueSize < maxFlushQueueSize){
		if (!it->second){
			flushScheduled = true;
			addEvent(0, FLUSH);
		}
...
void HybridMemoryManager::finishFlushing(addrint srcPhysicalPage){
	...
	if (mig->second.rolledBack){
		...
		debug("\terasing from physicalPages: (%lu; %u, %lu)", ppit->first, ppit->second.pid, ppit->second.virtualPage);
		physicalPages.erase(ppit);
		debug("\tinserting into physical pages: (%lu; %u, %lu)", mig->second.destPhysicalPage, mig->second.pid, mig->second.virtualPage);
		...
		bool oldOriginalDemoted = mig->second.originalDemoted;

		migrations.erase(mig);

		memory->copyPage(it->second.page, destPhysPage);

		auto res = migrations.emplace(it->second.page, MigrationEntry(pid, virtualPage, destPhysPage, PCM, COPY, timestamp));
		myassert(res.second);
		res.first->second.originalRolledBack = true;
		res.first->second.originalDemoted = oldOriginalDemoted;
		...
		if (mig->second.state == FLUSH_BEFORE){
			...
			if (mig->second.dest == PCM){
				if (mig->second.originalRolledBack){
					if (mig->second.originalDemoted){
						myassert(demoting);
						demoting = false;
						debug("(): adding 1 DEMOTE event for %lu", timestamp+1);
						addEvent(1, DEMOTE);
					} else {
						myassert(rollingBack);
						rollingBack = false;
					...
					myassert(demoting);
					demoting = false;
					debug("(): adding 2 DEMOTE event for %lu", timestamp+1);
					addEvent(1, DEMOTE);
				}
			}
			if (mig->second.completing){
				completing = false;
				addEvent(1, COMPLETE);
			}
...
void HybridMemoryManager::copyCompleted(addrint srcPhysicalPage){
	...
	if (mig->second.rolledBack){
		...
		if(mig->second.originalDemoted){
			myassert(demoting);
			demoting = false;
			debug("(): adding DEMOTE event for %lu", timestamp+1);
			addEvent(1, DEMOTE);
		} else {
			myassert(rollingBack);
			rollingBack = false;
...
void HybridMemoryManager::unstallCpus(unsigned pid, addrint virtualPage){
	//uint64 timestamp = engine->getTimestamp();
	//debug("(%u, %lu)", pid, virtualPage);
...
unsigned HybridMemoryManager::getPidOfAddress(addrint addr){
 ...
 */
bool OldHybridMemoryManager::access(unsigned pid, addrint virtualAddr, bool read, bool instr, addrint *physicalAddr, CPU *cpu){
	uint64 timestamp = engine->getTimestamp();
	//debug("(%u, %lu, %s, %s)", pid, virtualAddr, read ? "read" : "write", instr ? "instr" : "data");
	addrint virtualPage = getIndex(virtualAddr);
	PageMap::iterator it = pages[pid].find(virtualPage);
...
void OldHybridMemoryManager::finish(unsigned core){
	coresFinished.insert(core);
...
bool OldHybridMemoryManager::startMigration(unsigned pid){
...
void OldHybridMemoryManager::selectPolicyAndMigrate(){
	...
	if (state == NOT_MIGRATING){
		unsigned previousPolicy = currentPolicy;
		bool found = false;
		do {
...
void OldHybridMemoryManager::unstallCpus(unsigned pid, addrint virtualPage){
	debug("(%d, %lu)", pid, virtualPage);
	StalledCpuMap::iterator it = stalledCpus[pid].find(virtualPage);
	if (it != stalledCpus[pid].end()){
		for (list<CPU*>::iterator itList = it->second.begin(); itList != it->second.end(); itList++){
			(*itList)->resume(pid, virtualPage);
		}
		stalledCpus[pid].erase(it);
	}
...
unsigned OldHybridMemoryManager::getPidOfAddress(addrint addr){
	PhysicalPageMap::iterator it = physicalPages.find(getIndex(addr));
	if (it == physicalPages.end()){
		return -1;
	} else {
...
bool SimpleMemoryManager::access(unsigned pid, addrint virtualAddr, bool read, bool instr, addrint *physicalAddr, CPU *cpu){
...
void SimpleMemoryManager::finish(unsigned coreId){
