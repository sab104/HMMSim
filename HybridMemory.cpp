...
HybridMemory::~HybridMemory(){
	myassert(rolledBackMigrations.empty());
	myassert(callbacks.empty());
	myassert(stalledOnDramRead.empty());
	myassert(stalledOnDramWrite.empty());
	myassert(stalledOnPcmRead.empty());
	myassert(stalledOnPcmWrite.empty());
	myassert(dramStalledCallers.empty());
	myassert(pcmStalledCallers.empty());
...
bool HybridMemory::access(MemoryRequest *request, IMemoryCallback *caller){
	...
	debug("(%p, %lu, %lu, %u, %s, %s, %d, %s)", request, request->addr, page, request->size, request->read?"read":"write", request->instr?"instr":"data", request->priority, caller->getName());
	...
	if (mit != migrations.end()){
		debug(": startPageCopyTime: %lu", mit->second.startPageCopyTime);
		debug(": state == %d", mit->second.blocks[block].state);
		addrint srcPage, destPage;
		...
		if (mit->second.blocks[block].state == NOT_READ){
			...
			if(request->read){
				if (!mit->second.rolledBack){
					//info(": request->addr: %lu, manager->getAddressFromBlock(srcPage, block): %lu", request->addr, manager->getAddressFromBlock(srcPage, block));
					myassert(request->addr == manager->getAddressFromBlock(srcPage, block));
				}
				addrint origAddr = request->addr;
				request->addr = manager->getAddressFromBlock(srcPage, block);
				...
				if(accessNextLevel(request, caller, callbackAddr, true, page)){
					...
					if (mit->second.blocksLeftToRead == completionThreshold && mit->second.blocksLeftToRead > 0){
						...
						while (bit != mit->second.blocks.end() && bit->state != NOT_READ){
							++bit;
							mit->second.nextReadBlock = (mit->second.nextReadBlock + 1) % blocksPerPage;
						}
				...
				mit->second.blocks[block].ignored = true;
				mit->second.blocks[block].request = request;
				mit->second.blocksLeftToRead--;
				mit->second.blocksLeftToCompleteRead--;
				if (mit->second.blocksLeftToRead == completionThreshold && mit->second.blocksLeftToRead > 0){
					auto bit = mit->second.blocks.begin();
					while (bit != mit->second.blocks.end() && bit->state != NOT_READ){
						++bit;
		...
		if(page >= pcmPageOffset && caller != manager && manager->migrateOnDemand(page, &destPage)){
			...
			auto p = migrations.emplace(page, MigrationEntry(destPage, pcm, dram, &stalledOnPcmRead, &stalledOnDramWrite, dramMigrationReadDelay, dramMigrationWriteDelay, blocksPerPage, timestamp));
			myassert(p.second);
			...
			if (request->read){
				...
				p.first->second.blocks[block].ignored = true;
				p.first->second.blocks[block].request = request;
				p.first->second.blocksLeftToRead--;
				p.first->second.blocksLeftToCompleteRead--;
			...
			if (p.first->second.blocksLeftToRead == completionThreshold){
				...
				while (bit != p.first->second.blocks.end() && bit->state != NOT_READ){
					++bit;
					p.first->second.nextReadBlock = (p.first->second.nextReadBlock + 1) % blocksPerPage;
				}
	...
	if (caller != manager){
		...
		if (monit == monitors.end()){
			...
			//monit = monitors.emplace(page, CountEntry(page, blocksPerPage)).first;
		}
		if (read){
			monit->second.reads++;
			//monit->second.readBlocks[block]++;
			monit->second.lastReadTime = timestamp;
		} else {
			monit->second.writes++;
...
bool HybridMemory::accessNextLevel(MemoryRequest *request, IMemoryCallback *caller, addrint callbackAddr, bool partOfMigration, addrint srcPage){
	addrint page = manager->getIndex(request->addr);
	debug("(%p, %lu, %lu, %s, %lu, %s, %u, %lu)", request, request->addr, page, caller->getName(), callbackAddr, request->read ? "read" : "write", partOfMigration, srcPage);
	bool read = request->read;
	if (request->addr < pcmOffset){
...
void HybridMemory::accessCompleted(MemoryRequest *request, IMemory *caller){
	...
	addrint block = manager->getBlock(request->addr);
	addrint page = manager->getIndex(request->addr);
	debug("(%p, %lu, %lu, %s, %s)", request, request->addr, page, request->read ? "read" : "write", caller->getName());
	...
	if (partOfMigration){
		...
		debug(": \tstate: %u", mit->second.blocks[block].state);
		if (mit->second.blocks[block].state == NOT_READ){
			myassert(false);
		} else if(mit->second.blocks[block].state == READING){
			if(caller == mit->second.src){
				mit->second.blocks[block].state = BUFFERED;
				if (calledBack){
		...
		debug(": blocksLeftToRead: %u, blockLeftToCompleteRead: %u, blocksLeftToIgnore: %u, blocksLeftToWrite: %u", mit->second.blocksLeftToRead, mit->second.blocksLeftToCompleteRead, mit->second.blocksLeftToIgnore, mit->second.blocksLeftToWrite);
		if (mit->second.blocksLeftToCompleteRead == 0 && mit->second.blocksLeftToIgnore == 0 && mit->second.blocksLeftToWrite == 0){
...
void HybridMemory::copyPage(addrint srcPage, addrint destPage){
	...
	if (srcPage < pcmPageOffset){
		if (destPage < pcmPageOffset){
			...
			auto p = migrations.emplace(srcPage, MigrationEntry(destPage, dram, pcm, &stalledOnDramRead, &stalledOnPcmWrite, pcmMigrationReadDelay, pcmMigrationWriteDelay, blocksPerPage, timestamp));
			myassert(p.second);
...
void HybridMemory::finishMigration(addrint page){
	...
	if (mit->second.rolledBack){
		...
		auto sorit = mit->second.stalledOnRead->begin();
		while (sorit != mit->second.stalledOnRead->end()){
			if (*sorit == mit->first){
				sorit = mit->second.stalledOnRead->erase(sorit);
			} else {
				++sorit;
			}
		}
		auto sowit = mit->second.stalledOnWrite->begin();
		while (sowit != mit->second.stalledOnWrite->end()){
			if (*sowit == mit->first){
				sowit = mit->second.stalledOnWrite->erase(sowit);
			} else {
				++sowit;
			}
		}
...
void HybridMemory::complete(addrint srcPage){
	...
	debug(": blocksLeftToRead: %u, blocksLeftToCompleteRead: %u, blocksLeftToIgnore: %u, blocksLeftToWrite: %u", mit->second.blocksLeftToRead, mit->second.blocksLeftToCompleteRead, mit->second.blocksLeftToIgnore, mit->second.blocksLeftToWrite);
...
void HybridMemory::rollback(addrint srcPage){
	...
	mit->second.stalledOnRead = &stalledOnDramRead;
	mit->second.stalledOnWrite = &stalledOnPcmWrite;

	mit->second.blocksLeftToRead = 0;
	mit->second.blocksLeftToCompleteRead = 0;
	mit->second.blocksLeftToIgnore = 0;
	...
	for(auto bit = mit->second.blocks.begin(); bit != mit->second.blocks.end(); ++bit){
		debug(": block: %lu, state: %u, dirty: %u, ignored: %u", manager->getAddressFromBlock(srcPage, i), bit->state, bit->dirty, bit->ignored);
		i++;
	...
	debug(": blocksLeftToRead: %u, blockLeftToCompleteRead: %u, blocksLeftToIgnore: %u, blocksLeftToWrite: %u", mit->second.blocksLeftToRead, mit->second.blocksLeftToCompleteRead, mit->second.blocksLeftToIgnore, mit->second.blocksLeftToWrite);
	if (mit->second.blocksLeftToRead == 0 && mit->second.blocksLeftToWrite == 0 && mit->second.blocksLeftToCompleteRead == 0 && mit->second.blocksLeftToIgnore == 0){
		...
		if (mit != migrations.end()){
			if (mit->second.blocksLeftToRead > 0){
				...
				if (it != mit->second.blocks.end()){
					...
					if (mit->second.stalledOnRead->empty() && mit->second.src->access(it->request, this)){
						it->state = READING;
						...
						debug(": stalled on read");
						mit->second.stalledOnRead->emplace_back(mit->first);
					}
				}
			}
		}
			...
			if (mit->second.blocksLeftToWrite > 0){
				if (mit->second.nextWriteBlock != -1){
					...
					if(it->request == 0){
						...
						prevAddr = it->request->addr;
						prevRead = it->request->read;
					...
					if (mit->second.stalledOnWrite->empty() && mit->second.dest->access(it->request, this)){
						debug(": not stalled");
						if (mit->second.dest == dram){
							dramCopyWrites++;
						} else if (mit->second.dest == pcm){
						...
						if (mit->second.blocksLeftToWrite == 0 && mit->second.blocksLeftToCompleteRead == 0 && mit->second.blocksLeftToIgnore == 0){
							debug(": finish copy, src: %s, dest: %s", mit->second.src->getName(), mit->second.dest->getName());
						...
						if (created){
							...
							it->request->addr = prevAddr;
							it->request->read = prevRead;
						}
						debug(": stalled on write: %lu", mit->first);
						mit->second.stalledOnWrite->emplace_back(mit->first);
					}
				}
			}
		}
...
void HybridMemory::unstall(IMemory *caller){
	...
	if (caller == dram){
		...
		for (auto it = stalledOnDramRead.begin(); it != stalledOnDramRead.end();++it){
			debug(": adding READ event: %lu", *it);
			addEvent(2, READ, *it);
		}
		stalledOnDramRead.clear();
		for (auto it = stalledOnDramWrite.begin(); it != stalledOnDramWrite.end();++it){
			debug(": adding WRITE event: %lu", *it);
			addEvent(2, WRITE, *it);
		}
		stalledOnDramWrite.clear();
	} else if (caller == pcm){
		...
		//delay 2 cycles so that regular request have higher priority while unstalling
		for (auto it = stalledOnPcmRead.begin(); it != stalledOnPcmRead.end();++it){
			debug(": adding READ event: %lu", *it);
			addEvent(2, READ, *it);
		}
		stalledOnPcmRead.clear();
		for (auto it = stalledOnPcmWrite.begin(); it != stalledOnPcmWrite.end();++it){
			debug(": adding WRITE event: %lu", *it);
			addEvent(2, WRITE, *it);
		}
		stalledOnPcmWrite.clear();
	} else {
		myassert(false);
	}


//	auto rit = stalledOnRead.begin();
//	while (rit != stalledOnRead.end()){
//		auto mit = migrations.find(*rit);
//		myassert(mit != migrations.end());
//		if (mit->second.src == caller){
//			//delay 2 cycles so that regular request have higher priority while unstalling
//			addEvent(2, READ, *rit);
//			rit = stalledOnRead.erase(rit);
//
//		} else {
//			++rit;
//		}
//	}
//
//	auto wit = stalledOnWrite.begin();
//	while(wit != stalledOnWrite.end()){
//		auto mit = migrations.find(*wit);
//		myassert(mit != migrations.end());
//		if (mit->second.dest == caller){
//			//delay 2 cycles so that regular request have higher priority while unstalling
//			addEvent(2, WRITE, *wit);
//			wit = stalledOnWrite.erase(wit);
//		} else {
//			++wit;
//		}
//	}
...
void HybridMemory::readCountsAndProgress(addrint page, vector<CountEntry> *monitor, vector<ProgressEntry> *progress){
	auto monit = monitors.find(page);
	if (monit != monitors.end()){
		monitor->emplace_back(monit->second);
		monitors.erase(monit);
	}
	auto it = migrations.find(page);
	if (it != migrations.end()){
		progress->emplace_back(it->first, it->second.blocksLeftToRead);
	}
}
...
void HybridMemory::readCountsAndProgress(vector<CountEntry> *monitor, vector<ProgressEntry> *progress){
	...
	for (auto it = migrations.begin(); it != migrations.end(); ++it){
		progress->emplace_back(it->first, blockSize - it->second.blocksLeftToRead);
	}
}
