...
Memory::Memory(
	...
	addrint offsetArg) :
		...
		mapping(mappingTypeArg, numChannelsArg, ranksPerChannelArg, banksPerRankArg, rowsPerBankArg, blocksPerRowArg, blockSizeArg),
		offset(offsetArg),
		stalled(false),
		...
		numReadRequests(statCont, nameArg + "_read_requests", "Number of " + descArg + " read requests"),
		numWriteRequests(statCont, nameArg + "_write_requests", "Number of " + descArg + " write requests"),
		readQueueTime(statCont, nameArg + "_read_queue_time", "Number of cycles " + descArg + " read requests wait in the queue"),
		writeQueueTime(statCont, nameArg + "_write_queue_time", "Number of cycles " + descArg + " write requests wait in the queue"),
		readTotalTime(statCont, nameArg + "_read_total_time", "Total number of cycles of " + descArg + " read requests"),
		writeTotalTime(statCont, nameArg + "_write_total_time", "Total number of cycles of " + descArg + " write requests"),
		rowBufferHits(statCont, nameArg + "_row_buffer_hits", "Number of " + descArg + " row buffer hits"),
		rowBufferMisses(statCont, nameArg + "_row_buffer_misses", "Number of " + descArg + " row buffer misses"),
		numOpens(statCont, nameArg + "_num_opens", "Number of " + descArg + " opens"),
		numAccesses(statCont, nameArg + "_num_accesses", "Number of " + descArg + " accesses"),
		numCloses(statCont, nameArg + "_num_closes", "Number of " + descArg + " closes"),
		numRARs(statCont, nameArg + "_num_read_after_read", "Number of " + descArg + " read after read (RAR) hazards"),
		numRAWs(statCont, nameArg + "_num_read_after_write", "Number of " + descArg + " read after write (RAW) hazards"),
		numWARs(statCont, nameArg + "_num_write_after_read", "Number of " + descArg + " write after read (WAR) hazards"),
		numWAWs(statCont, nameArg + "_num_write_after_write", "Number of " + descArg + " write after write (WAW) hazards"),
		waitLowerPriorityTime(statCont, nameArg + "_wait_lower_priority_time", "Number of cycles " + descArg + " requests wait for lower priority requests"),
		waitSamePriorityTime(statCont, nameArg + "_wait_same_priority_time", "Number of cycles " + descArg + " requests wait for same priority requests"),
		waitHigherPriorityTime(statCont, nameArg + "_wait_higher_priority_time", "Number of cycles " + descArg + " requests wait for higher priority requests"),
...

{
	...
	banks.resize(mapping.getNumChannels());
	for (unsigned i = 0; i < mapping.getNumChannels(); i++) {
		stringstream ssName;
		ssName << name;
		ssName << "_bus_" << i;
		stringstream ssDesc;
		ssDesc << desc;
		ssDesc << " bus " << i;
		Bus *newBus = new Bus(ssName.str(), ssDesc.str(), engineArg, statCont, debugStartArg, busLatencyArg);
		buses.push_back(newBus);
		banks[i].resize(mapping.getNumBanksPerChannel());
		for (unsigned j = 0; j < mapping.getNumBanksPerChannel(); j++) {
			stringstream ssName;
			ssName << name;
			ssName << "_channel_" << i;
			ssName << "_bank_" << j;
			...
			ssDesc << " channel " << j;
			ssDesc << " bank " << j;
			Bank *newBank = new Bank(ssName.str(), ssDesc.str(), engineArg, statCont, debugStartArg, queueCounterIndexArg, openCounterIndexArg, accessCounterIndexArg, closeCounterIndexArg, busQueueCounterIndexArg, busCounterIndexArg, policyArg, typeArg, this, newBus, openLatencyArg,
	...
	if (globalQueue) {
		queueSizes.resize(mapping.getNumChannels());
		for (unsigned i = 0; i < mapping.getNumChannels(); i++) {
			queueSizes[i].push_back(0);
		}
	} else {
		queueSizes.resize(mapping.getNumChannels());
		for (unsigned i = 0; i < mapping.getNumChannels(); i++) {
			queueSizes[i].resize(mapping.getNumBanksPerChannel());
			for (unsigned j = 0; j < mapping.getNumBanksPerChannel(); j++) {
				queueSizes[i][j] = 0;
			}
		}
...
Memory::~Memory(){
	...
	if (globalQueue){
		for (unsigned i = 0; i < mapping.getNumChannels(); i++) {
			myassert(queueSizes[i][0] == 0);
		}
	} else {
		for (unsigned i = 0; i < mapping.getNumChannels(); i++) {
			for (unsigned j = 0; j < mapping.getNumBanksPerChannel(); j++) {
				myassert(queueSizes[i][j] == 0);
			}
		}
	}
	myassert(requests.empty());
	myassert(stalledCallers.empty());
}
...
bool Memory::access(MemoryRequest *request, IMemoryCallback *caller){
	...
	unsigned channelIndex = mapping.getChannelIndex(request->addr);
	...
	debug(": channelIndex: %u, queueIndex: %u, queueSize: %d", channelIndex, queueIndex, queueSizes[channelIndex][queueIndex]);

	requests.emplace(request, caller);

	banks[channelIndex][bankIndex]->access(request, this);

	myassert(queueSizes[channelIndex][queueIndex] < maxQueueSize);
	queueSizes[channelIndex][queueIndex]++;
	if (queueSizes[channelIndex][queueIndex] == maxQueueSize) {
		stalled = true;
...
void Memory::accessCompleted(MemoryRequest *request, IMemory *caller) {
	...
	myassert(queueSizes[channelIndex][queueIndex] > 0);
	queueSizes[channelIndex][queueIndex]--;
}
...
bool CacheMemory::access(MemoryRequest *request, IMemoryCallback *caller){
	...
	if (!found){
		...
		if (it == internalRequests.end()){
			...
			debug(": %s, evictedAddr: %lu", it->second.result == CacheModel::HIT ? "hit" : (it->second.result == CacheModel::MISS_WITHOUT_EVICTION ? "miss without eviction" : (it->second.result == CacheModel::MISS_WITH_EVICTION ? "miss with eviction" : (it->second.result == CacheModel::MISS_WITH_WRITEBACK ? "miss with writeback" : ("")))), it->second.evictedAddr);
...
void CacheMemory::process(const Event *event){
	...
	debug("(): eventType: %u", eventType);
	if (eventType == TAG_ARRAY){
		addrint blockAddr = data & ~eventTypeMask;
		InternalRequestMap::iterator it = internalRequests.find(blockAddr);
		myassert(it != internalRequests.end());
		debug(": result: %u", it->second.result);
		if (it->second.result == CacheModel::HIT){
...
void CacheMemory::accessDram(MemoryRequest *request, addrint internalRequestAddr, bool delay){
	...
	if(delay){
		...
		debug(": dram.access(%lu, %s)", request->addr, request->read?"read":"write");
		if(!stalledDramRequests.empty() || !dram->access(request, this)){
			debug(": dram is stalled");
...
void CacheMemory::accessPcm(MemoryRequest *request, addrint internalRequestAddr, bool delay){
	...
	if(delay){
		...
		debug(": pcm.access(%lu, %s)", request->addr, request->read?"read":"write");
		if(!stalledPcmRequests.empty() || !pcm->access(request, this)){
			debug(": pcm is stalled");
