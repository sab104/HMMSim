...
CacheModel::CacheModel(const string& nameArg, const string& descArg, StatContainer *statCont, uint64 cacheSizeArg, unsigned blockSizeArg, unsigned setAssocArg, CacheReplacementPolicy policyArg, unsigned pageSizeArg) :
	...
	misses(statCont, nameArg + "_all_misses", "Number of " + descArg + " misses", &misses_without_eviction, &misses_with_eviction, &misses_with_writeback, &misses_without_free_block),
	accesses(statCont, nameArg + "_accesses", "Number of " + descArg + " accesses", &hits, &misses),
...
Cache::~Cache() {
	...
	myassert(flushRequests.empty());
	myassert(remapRequests.empty());
	myassert(tagChangeRequests.empty());
	myassert(outgoingFlushRequests.empty());
	myassert(outgoingRemapRequests.empty());
...
void Cache::flush(addrint blockAddr, uint8 size, bool guarantee, IFlushCallback *caller){
	...
	auto it = requests.find(blockAddr);
	if (it != requests.end()){
		res.first->second.repeat = true;
	}
	for (auto rit = requests.begin(); rit != requests.end(); ++rit){
		if (rit->second.result == CacheModel::MISS_WITH_WRITEBACK && rit->second.evictedAddr == blockAddr){
			res.first->second.dirty = true;
			rit->second.result = CacheModel::MISS_WITHOUT_EVICTION;
		}
	}
...
void Cache::flushCompleted(addrint blockAddr, bool dirty, IMemory *caller){
	...
	if (outIt->second.notificationsLeft == 0){
		if (outIt->second.dirty){
			auto fit = flushRequests.find(outIt->first);
			if (fit != flushRequests.end()){
				fit->second.dirty = true;
				outIt->second.dirty = false;
			}
		}
